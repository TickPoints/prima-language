...
fn eval(src: &str) -> Value {
    Evaluator::new().eval_value(src).unwrap_or_else(|e| panic!("eval failed for {src:?}: {e}"))
}
...
fn dict_index_membership_and_keys() {
    ...
    assert_eq!(eval("let d = { \"a\": 1, \"b\": 2 };\n\"x\" in d"), Value::Bool(false));
    assert_eq!(eval("let d = { \"a\": 1, \"b\": 2 };\n\"a\" in d"), Value::Bool(true));
    assert_eq!(eval("let d = { \"a\": 1, \"b\": 2 };\nd.keys()"), Value::Array(vec![
        Value::String("a".into()),
        Value::String("b".into()),
    ]));
    assert_eq!(eval("let d = { \"a\": 1, \"b\": 2 };\nd.keys().len()"), n(2));
    assert!(Evaluator::new().eval_value("let d = { \"a\": 1 };\nd[\"x\"]").is_err());
}
...
fn array_comprehension() {
    assert_eq!(eval("[x^2 for x in range(0, 5)]"), Value::Array(vec![
        n(0),
        n(1),
        n(4),
        n(9),
    ...
    assert_eq!(eval("[x for x in range(0, 6) if x % 2 == 0]"), Value::Array(vec![
        n(0),
        n(2),
        n(4),
    ]));
...
fn convenience_functions() {
    ...
    assert_eq!(eval("sorted([3, 1, 2])"), Value::Array(vec![n(1), n(2), n(3)]));
...
fn negative_index_and_slices() {
    ...
    assert_eq!(eval("[10, 20, 30, 40][1..3]"), Value::Array(vec![n(20), n(30)]));
    assert_eq!(eval("[10, 20, 30, 40][..2]"), Value::Array(vec![n(10), n(20)]));
    assert_eq!(eval("[10, 20, 30, 40][-2..]"), Value::Array(vec![n(30), n(40)]));
...
fn array_methods() {
    ...
    assert_eq!(eval("let v = [1, 2, 3];\nv.first()"), Value::Option(Some(Box::new(n(1)))));
    assert_eq!(eval("let v = [1, 2, 3];\nv.last()"), Value::Option(Some(Box::new(n(3)))));
    assert_eq!(eval("let v = [1, 2, 3];\nv.get(-1)"), Value::Option(Some(Box::new(n(3)))));
    assert_eq!(eval("let v = [1, 2, 3];\nv.pop()"), Value::Option(Some(Box::new(n(3)))));
    assert_eq!(eval("let v = [3, 1, 2];\nv.sort();\nv"), Value::Array(vec![n(1), n(2), n(3)]));
    assert_eq!(eval("let v = [1, 2, 3];\nv.reverse();\nv"), Value::Array(vec![n(3), n(2), n(1)]));
}
...
fn string_join_strip_find() {
    assert_eq!(eval("\"-\".join([\"a\", \"b\", \"c\"])"), Value::String("a-b-c".into()));
    assert_eq!(eval("join([\"a\", \"b\", \"c\"], \"-\")"), Value::String("a-b-c".into()));
    assert_eq!(eval("let s = \"  hi  \";\ns.strip(\" \")"), Value::String("hi".into()));
    assert_eq!(eval("let s = \"hello\";\ns.find(\"ll\")"), Value::Option(Some(Box::new(n(2)))));
    assert_eq!(eval("let s = \"hello\";\ns.find(\"x\")"), Value::Option(None));
}
