    ...
    pub fn to_value(&self) -> Value {
        match self {
            ...
            ValueKey::Rational(n, d) => {
                Value::Number(Number::Rational(BigRational::new(n.clone(), d.clone())))
            }
...
fn number_to_key(n: &Number) -> Option<ValueKey> {
    ...
    match n {
        ...
        Number::Real(Real::F32(x)) => {
            (!x.is_nan()).then_some(ValueKey::Float((*x as f64).to_bits()))
        }
...
mod tests {
    ...
    fn bigint_key_roundtrip() {
        ...
        assert_roundtrip(
            Value::Number(Number::Integer(big.clone())),
            ValueKey::BigInt(big),
        );
    }
    ...
    fn unsupported_values_are_none() {
        ...
        assert_eq!(
            ValueKey::from_value(&Value::Array(vec![Value::Number(Number::from(1))])),
            None
        );
    ...
    fn dict_and_set_hold_hashable_keys() {
        ...
        let Value::Dict(d) = Value::Dict(m) else {
            unreachable!()
        };
        assert_eq!(
        ...
        let Value::Set(s) = Value::Set(s) else {
            unreachable!()
        };
        assert_eq!(s.len(), 2);
