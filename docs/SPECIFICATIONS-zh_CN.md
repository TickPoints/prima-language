 ...
 **W_numeric** | 数值世界 | 基本数值（i8…f64、BigInt、复数）、矩阵、数组 | 栈上值类型 | 栈 + 线性内存（BLAS） | 原生速度 |
  **W_host** | 宿主世界 | 控制流、Class 对象、I/O | 用户对象（类实例） | 引用计数（`Rc<RefCell>`，浅拷贝共享）+ 值语义（深拷贝）；`mem::Arc` 显式引用计数（§12.4） | 功能性 |
...
**类实例所有权**（引用计数语义；需要确定性生命周期时可整体持托管 `mem::Arc`（§12.4））：

- **默认值语义**：类实例由宿主层**引用计数**（`Rc<RefCell>`）管理；**赋值、传参、返回实例**均为**浅拷贝**（共享底层对象的句柄，自带计数开销）。
...
- **循环引用**：引用计数无法回收含循环引用的实例；需要确定性生命周期或跨 FFI 保持存活时，使用标准库 `mem::Arc`（打包一层显式引用计数，`mem::Arc::new(x)`/`x.strong_count()`）。
...
----|------|
 ...
 W_host | **引用计数**（`Rc<RefCell>`）管理类实例（浅拷贝共享 + 基本值深拷贝）；`String` 缓冲内联/SSO；`mem::Arc` 供显式引用计数 |

> 注：宿主层不采用追踪式 GC。参考计数已覆盖类实例的浅拷贝共享；`mem::Arc` 提供显式的引用计数路径（确定性释放/跨 FFI），二者均不引入析构钩子（§12.3）。
 ...
 **mem** | **内存（v2.2）**：`mem::Arc` 显式引用计数（§12.3/12.4） | 显式 |
...
---|------|------|
 ...
 5 | 内存 | hash-consing 符号层（线程本地缓存 + 全局池）+ 栈值数值层 + 引用计数宿主层（§8.1/12.4） |
 ...
 53 | 内存（v2.2） | **宿主层引用计数（`Rc<RefCell>`）管理类实例；`mem::Arc` 提供显式引用计数**（§12.3/12.4） |
