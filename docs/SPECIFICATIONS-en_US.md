# **Prima** — Language Specification v2.1

> **Translation note**: this is the English counterpart of the authoritative Chinese specification [`SPECIFICATIONS-zh_CN.md`](./SPECIFICATIONS-zh_CN.md) (v2.1). The Chinese original is the final authority; this translation may lag or differ on the margins.
> **Notice**: This specification is the official language specification v2.1 of the **Prima language** and is the final authority for unified design and implementation.
...
| **W_host** | Host World | Control flow, Class objects, I/O | user objects (class instances) | reference counting (shallow-copy sharing) + value semantics (deep copy) | functional |
...
- **Strings**: `"..."` (with escapes, including `\u{XXXX}`) + raw strings `r"..."`.
- **TeX literals**: ``tex"..."``.
...
**Principle**: all optimizations happen **automatically**; no optimization directives are exposed to the developer (no annotations like `#[inline]`). `@parallel`/`@jit`/`@gpu` are **parallel/execution-model** annotations, not optimization directives.
...
7. **Simplification level**: coordinates with the simplification system of §8.3; the `simplify_level` policy controls symbolic-simplification depth, with numeric optimization following after.
...
**Class instance ownership** (analogous to Rust `Arc`/`&self`, hidden from the developer):

- **Default value semantics**: class instances are managed by reference counting; **assignment, argument passing, and returning an instance** are all **shallow copies** (share the underlying object, refcount +1).
...
----|------|
  ...
  W_host | reference counting for class instances (shallow-copy sharing + deep copy for primitive values); `String` buffers inline/SSO |
...
------|------|------|------|
  ...
  `simplify_level` | int 0-3 | `2` | Default simplification level |
...
### 15.5 Pre-imports
- **`core` is pre-imported**, and **all common `core` features (built-in symbols, the collapse-function family, basic operators, `format`, `Option`/`Result` variants) are fully exposed**.
- All other modules (`linalg`/`stats`/`plot`/`math`/`io`/`parallel`/`physics`/`sys`/`time`/`num`/`ops`/`c_api`) must be explicitly `import`ed.
...
- Utility functions: `format`, `range`, `map`, `filter`, `Some`, `None`, `Ok`, `Err`
...
- Deprecation warnings (`W0001`/`W0002`) are removed along with the corresponding syntax in the target version.
...
------|------|------|
 **core** | Number tower, ExprDAG, simplification, built-in symbols, the collapse function family, basic operators, `format`, `Result`/`Option` | **Pre-imported; all common names exposed** |
 ...
 **plot** | Plotting (SVG to start, optional matplotlib-rs backend) | Explicit |
 **math** | Special functions (Bessel, gamma, hypergeometric), numerical integration, ODE, FFT | Explicit |
 ...
 **physics** | Physical constants (CODATA 2022), unit system (optional) | Explicit |
 ...
 **sys** | Low-level system operations: `sys::path` (cross-platform paths), `sys::env` (environment), `sys::os` (platform-specific) | Explicit |
...
### 18.1 Strings (the core-preimported `String` class and `format`)

**Literals**: `"..."` (escaped), `r"..."` (raw string), and `\u{XXXX}` Unicode escapes inside `"..."`.
...
**The `String` class** (`@builtin`, implemented in Rust):
```prima
@builtin
pub class String {
    pub fn new() -> Self
...
**`format` rules**:
- `{}` takes arguments by position; `{0}`/`{1}` take arguments by index (optional).
- When an argument is an `Expr`, it is rendered according to the `print_format` policy (default LaTeX).
- When an argument is a `Result`/`Option`, its success/failure summary is shown.
- Refinements such as `{:format}` are supported (e.g. `{:0.2}` for float precision), being extended gradually.
...
Applied to `fn`/`class` declarations without a body (signature only), indicating the implementation is provided by the Rust host; at runtime the name is bound to a registered builtin implementation. If unregistered, a compile-time error is raised (`E0055`).
...
---|------|------|
 ...
 36 | Strings | **Full escaping (including `\u{XXXX}`), `format("...{}...")`, the `String` class method set** (§18.1) |
 ...
 39 | Optimization | **Automatic inlining (no developer control) + a modern optimization pipeline: constant folding/CSE/loop optimization/TCO, etc.** (§10.2) |
 40 | Operator overloading | **`impl` in the `ops` module; `overload_policy` defaults to `warn` (W0005)** (§18.5) |
 41 | Standard library extensions | **`sys` (path/env/os), `time`, `num`, `ops`, `c_api`** (§18) |
...
----|------|------|
 ...
 `E0055` | `unregistered_builtin` | `@builtin` without a registered implementation |
 `E0056` | `builtin_body` | `@builtin` functions/classes must not have a body |
...
*Prima language specification v2.1 · the authoritative basis for the design and implementation of Prima*
