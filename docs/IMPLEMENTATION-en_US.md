...
> **Phase 6 landing record (2026-08, v2.2)**: all complete. Deviations/final decisions vs this document:
>
> - **`'...'` Char/String disambiguation**: appendix A's `char ::= "'" character "'"` and `string ::= "'" char* "'"` both match a single character. Final decision: a single character (after escapes) → `Char`; multiple characters or the empty string → single-quoted string (keeps the existing `Char` literal spelling, and char range patterns `'a'..'z'` keep working; §4.4).
> - **f-string lexing**: the lexer scans the template in `read_fstring`, folding `{{`/`}}` into literal segments; `{expr[:spec]}` bodies are scanned by `read_fstring_interp` (tracking `{}`/`()`/`[]` depth and nested strings; at depth 0 a `:` that is not `::` starts the `:spec`). Expression bodies are re-lexed by `lex_range` into a token stream with **absolute spans**, which the parser parses with a nested `Parser`. A nested f-string is detected in the sub-token stream and reported as a compile-time error (§18.1). f-string literal segments are not escape-processed beyond `{{`/`}}` (the `raw` bit), while interpolation bodies always parse escapes (Python semantics).
> - **`W0006` is emitted by the parser** (pushed in `parse_postfix` for a bare `format(...)` call): it flows through both the `prima check` warning channel and evaluator warnings (`parse_checked` feeds `Evaluator.warnings`); module functions (`time::format`) do not trigger it. `format` was removed from `Builtin::from_name`, the `Env::core` pre-import, and `collapse::call`; calling it now fails as an unknown name.
> - **`{:spec}` subset**: `[[fill]align][width][.precision]` plus the leading-`0` zero-pad flag (Python `format` mini-language); precision applies only to `Value::Number::Real` (F64/F32), other values ignore precision and keep the rendered text; numbers/zero-padding default to right alignment, otherwise left. `apply_spec` lives in `eval.rs`.
> - **`Literal::Str` renamed `Literal::String { value, quote, raw }`**; `TokenKind::Str` likewise becomes a struct. `prima fmt` preserves the original `r"..."`/`'...'` spelling for `Literal::String` (falling back to the escaped form when the value contains the delimiter) and re-emits f-strings in escaped canonical form; both pass the idempotence test.
> - **`\a \b \f \v` escapes added** (full §18.1 escape table); `read_quoted`/`read_escape`/`read_unicode_escape` are split out and shared by ordinary/raw/f-string reading.

> **Phase 7 landing record (2026-08, v2.2)**: all complete. Deviations/final decisions vs this document:
>
> - **Lexing/parsing**: the lexer gains `TokenKind::Doc { text, module }` (`///`=item, `//!`=module; one optional leading space stripped after the marker); the parser's `take_docs` collects consecutive doc lines (blank lines do not break the run, §4.1 merge semantics) into a `DocComment` attached to `Program.module_docs`/`Import.docs`/`Stmt::{Let,Const,FnDef,MathDef,ClassDef}.docs`/`ClassMember.docs` (§4.2). `//!` is only valid at the file top (before `config`/`import`); a dangling `///` (no following item) or a misplaced `//!` warns **`W0007 unattached_doc_comment`** (new §16.5 code). `prima fmt` preserves and re-emits doc comments (idempotent).
> - **Data plane**: `ClassDef.docs`/`MethodDef.docs` are copied from the AST; a new `prima-runtime::docs` registry (`register_doc`/`get_doc`/`class_methods`, keys `"String::to_upper"`, class-level `"String"`) is seeded at startup by `prima-stdlib::init` parsing `core/string.pra`. `prima-stdlib` gains a direct `prima-syntax` dependency (to parse the embedded source).
> - **Diagnostic notes** (§16.4): `RuntimeError::WithNotes { notes, help, error }` wraps an error; `notes()`/`help()` walk the chain. A failed method call (unknown method / wrong arity / visibility / runtime error inside the body) and failed `T::name` associated calls attach a **signature + definition location + `///` doc** note and a `did you mean` help (Levenshtein ≤ 2 or shared prefix); unknown native `String`/`Array`/`Dict`/`Set` methods fall back to the class-level doc. `prima check`'s `E0050` errors (`TypeError.notes`) attach the stdlib function signature + definition location + doc. **User-class method notes carry no line number** (the runtime class model records only the module path, not a source location; stdlib has the full `core/string.pra:<line>:<col>`).
> - **`prima doc`**: now renders Markdown from the AST `docs` (`#` module title + `//!` + `##` per item + inline class-member docs); gains `-o FILE` and `--stdlib` (enumerates every embedded module via `stdlib::all_module_sources`, offline-queryable, §16.4).
> - **`core/string.pra`**: a `class String` doc module covering the current native method set (`len`/`is_empty`/`push`/`insert`/`char_at`/`substring`/`contains`/`starts_with`/`ends_with`/`replace`/`trim`/`strip`/`split`/`join`/`find`/`to_upper`/`to_lower`/`repeat` + associated `new`); `from` is a reserved word so it stays out of the members and is documented in the `//!` header. Single-type-parameter `Result<T>` does not parse, so `insert` is declared `Result<String, String>`.
...
| Appendix A `char`/`string` (v2.2) | `'a'` matches both the `char` and `string` productions (BNF ambiguity) | **Single character → `Char`; multiple characters or empty → single-quoted string** | Keeps a literal spelling for `Value::Char` (including range patterns, §4.4); multi-character `'...'` is equivalent to `"..."` per §18.1 |
