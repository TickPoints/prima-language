...
fn parfor_writes_index_slots() {
    assert_eq!(
        eval_array("let A = [0, 0, 0, 0, 0];\nparfor i in 0..5 {\n    A[i] = i * i;\n}\nA"),
        [0, 1, 4, 9, 16]
            .into_iter()
            .map(prima_core::Number::from)
            .collect::<Vec<_>>()
    );
}
...
fn parallel_broadcast_small_array_sequential_equivalence() {
    ...
    assert_eq!(
        eval_array("let f(x) @parallel = x^2 + 1;\nlet v = [1, 2, 3];\nf(v)"),
        [2, 5, 10]
            .into_iter()
            .map(prima_core::Number::from)
            .collect::<Vec<_>>()
