...
fn dict_mutation_and_views() {
    ...
    assert_eq!(
        eval("let d = { \"a\": 1, \"b\": 2 };\nd.keys()"),
        arr(vec![Value::String("a".into()), Value::String("b".into())])
    );
    assert_eq!(
        eval("let d = { \"a\": 1, \"b\": 2 };\nd.values()"),
        arr(vec![
...
fn dict_and_set_comprehension() {
    ...
    assert_eq!(d.len(), 5);
    assert_eq!(
        d[&prima_core::ValueKey::Int(3)],
        Value::Number(Number::from(9))
    );
    assert_eq!(
        eval("{x for x in range(0, 10) if x % 2 == 1}.len()"),
        Value::Number(Number::from(5))
    );
    assert_eq!(
...
fn string_strip_join_find() {
    ...
    assert_eq!(
        ...
        Value::Option(Some(Box::new(Value::Number(Number::from(6)))))
    );
    assert_eq!(
        eval("let s = \"hello\";\ns.find(\"zzz\")"),
        Value::Option(None)
