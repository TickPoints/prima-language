...
fn c_arg_push(name: &str, cty: &str) -> String {
    match cty {
        "bool" => {
            format!("    args.push(Value::Number(Number::Real(Real::F64(if {name} {{ 1.0 }} else {{ 0.0 }}))));\n")
        }
...
fn call_body(e: &prima_runtime::capi::CExtern) -> String {
    let call = format!("call_file_export(Path::new(PRIMA_SRC_PATH), {:?}, args)", e.name);
    if e.ret == "void" {
        format!(
...
fn gen_wrapper(e: &prima_runtime::capi::CExtern) -> String {
    ...
    s.push_str(&format!("pub extern \"C\" fn {}({}) -> {ret} {{\n", e.name, params.join(", ")));
...
fn gen_lib_source(exports: &[prima_runtime::capi::CExtern], src_path: &str) -> String {
    ...
    s.push_str("//!\n");
    s.push_str("//! Each `#[unsafe(no_mangle)] pub extern \"C\"` wrapper marshals the C arguments into\n");
    s.push_str("//! `prima_core::Value`s, evaluates the exporting `.pra` module through\n");
    s.push_str("//! `prima_runtime::capi::call_file_export` (spec §18.4), and converts the result back\n");
    ...
    s.push_str("/// Absolute path of the `.pra` module whose exports this library wraps. If the file is\n");
    ...
    s.push_str("// String results must outlive the wrapper's return frame; keep the buffers in a\n");
    s.push_str("// thread-local that is replaced on every call (previous buffers stay alive until the\n");
    s.push_str("// next call).\n");
    s.push_str("thread_local! {\n");
    s.push_str("    static CSTR_KEEP: RefCell<Vec<CString>> = const { RefCell::new(Vec::new()) };\n");
    s.push_str("}\n\n");
...
fn find_artifact(target_dir: &Path, crate_name: &str) -> Option<PathBuf> {
    ...
    for entry in entries.flatten() {
        ...
        let Some((stem, e)) = name.split_once('.') else { continue };
        if e != ext {
            continue;
        }
...
pub(crate) fn run(file: &Path, output: Option<&Path>) -> ExitCode {
    ...
    let base = output.map(|p| p.to_path_buf()).unwrap_or_else(|| PathBuf::from("libprima_export"));
    let stem = base
        .file_stem()
    ...
    if let Err(e) = std::fs::write(&header_path, prima_runtime::capi::render_header(&exports)) {
        crate::diagnostics::print_colored_error(&format!("cannot write {}: {e}", header_path.display()));
        return ExitCode::FAILURE;
    }
    ...
    if !write("Cargo.toml", &gen_cargo_toml(&crate_name, &runtime_path, &core_path))
        || !write("src/lib.rs", &gen_lib_source(&exports, &src_path))
    {
        crate::diagnostics::print_colored_error("cannot write the export shell crate to the temp dir");
